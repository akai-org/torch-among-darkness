...
namespace UnityHFSM.Samples.RangeAI {
...
public class RangeAI : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        fightFsm.AddState("Wait", onEnter: state => Debug.Log("idle anim"));
    ...
    void SpawnBullet(){
        Rigidbody2D bulletInstance = Instantiate(bullet, transform.position, transform.rotation).GetComponent<Rigidbody2D>() as Rigidbody2D;
        bulletInstance.AddForce(Vector3.MoveTowards(
            transform.position,
            playerPosition,
            Mathf.Max(0, Mathf.Min(2 * Time.deltaTime, Vector2.Distance(transform.position, playerPosition)))) * 10f);
    }
