...
public class RangeEnemy : EnemyCharacterController
{
    ...
    [SerializeField] private float searchTime = 20;  // in seconds++
    ...
    new void Start()
    {
        ...
        var fightFsm = new HybridStateMachine(
            beforeOnLogic: state => MoveTowards(playerPosition, attackSpeed, minDistance: 1, false),
            needsExitTime: true
        );
        ...
        fsm.AddState("Chase", new State(
            onLogic: state => {
            MoveTowards(playerPosition,speed, 0, true);
            //Debug.Log("test");
            }
        ));
    ...
    private void MoveTowards(Vector2 target, float speed, float minDistance=0, bool chasePalyer = false)
    {
        //Debug.Log(chasePalyer);
        float vel = chasePalyer ? -Mathf.Max(0, Mathf.Min(speed * Time.deltaTime, Vector2.Distance(transform.position, target) - minDistance)) : Mathf.Max(0, Mathf.Min(speed * Time.deltaTime, Vector2.Distance(transform.position, target) - minDistance));
        //Debug.Log(target);
    ...
    private IEnumerator MoveToPosition(Vector2 target, float speed, float tolerance=0.05f, bool searchPlayer = false)
    {
        while (Vector2.Distance(transform.position, target) > tolerance)
        {
            MoveTowards(target, speed,0 , searchPlayer);
            // Wait one frame.
            yield return null;
        }
    }
    ...
    {
        yield return MoveToPosition(lastSeenPlayerPosition, speed, 0.05f, true);
